...
namespace lubee {
	double VanDerCorput(uint32_t i, uint32_t base) {
		double h = 0,
			   f = 1.0 / base,
			   factor = f;
	...
	namespace detail {
		...
		void CheckDim(const std::size_t dim) {
			D_Assert(dim <= N_Primes, "too big dimension");
		}
	}
	void Halton(double* dst, const uint32_t i, const std::size_t dim) {
		detail::CheckDim(dim);
	...
	Double_V Hammersley(const std::size_t n, const std::size_t dim) {
		detail::CheckDim(dim);
