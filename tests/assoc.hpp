...
namespace lubee {
	namespace test {
		namespace { namespace asv_inner {
			...
			bool _RemRandom(const int idx, A& a, Ts&... ts) {
				if(a.empty())
					return false;
				auto itr = a.cbegin();
				std::advance(itr, idx);
		...
		void AddRandom(const int n, RDF& rdF, AssocVec<V,C>& a, Ts&... ts) {
		...
		void AddRandom(const int n, RDF& rdF, AssocVecK<K,V,C>& a, Ts&... ts) {
